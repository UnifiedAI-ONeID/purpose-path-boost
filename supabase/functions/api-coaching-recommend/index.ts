...
Deno.serve(async (req) => {
  ...
  try {
    const { tags = [], lang = 'en' } = req.method === 'POST' 
      ? await req.json() 
      : { 
          tags: new URL(req.url).searchParams.get('tags')?.split(',') || [],
          lang: new URL(req.url).searchParams.get('lang') || 'en'
        };
    ...
    const scored = (offers || []).map(o => {
      ...
      const score = offerTags.reduce((sum: number, t: string) => {
        ...
        if (tags.some((x: string) => t.includes(x) || x.includes(t))) return sum + 1; // Partial match
        return sum;
      }, 0);
      
      const localized = pickFields(o, lang as Lang);
