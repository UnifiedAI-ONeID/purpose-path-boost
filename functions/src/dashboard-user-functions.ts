 ...
 */
export const dashboardUserSummary = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
 ...
 */
export const dashboardUserAnalytics = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
...
function determineNextBestAction(params: {
  ...
  habits: number;
  planSlug: string;
}): { title: string; cta: string; href: string; reason: string } {
