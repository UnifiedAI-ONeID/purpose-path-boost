 ...
 */
export const pwaBoot = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
  ...
  try {
    ...
    const recentProgress = progressSnap.docs.map(doc => ({
      lessonId: doc.id,
      ...doc.data()
    }));
 ...
 */
export const pwaQuizAnswer = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
  ...
  try {
    ...
    if (isLastQuestion) {
      ...
      const answers = answersSnap.docs.map(doc => ({
        ...
        ...doc.data()
      })) as Array<{ questionId: string; answer: unknown }>;
 ...
 */
export const pwaAiSuggest = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
  ...
  try {
    ...
    const suggestions = await generateAiSuggestions(prompt, {
      quizProfile: userData.quiz_profile,
      goals: userData.current_goals,
      ...userContext
    });
 ...
 */
export const pwaCoachingRecommend = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
  ...
  try {
    ...
    const recommendations = calculateCoachingRecommendations(
      packages,
      userData.quiz_profile,
      userData.lesson_history
    );
 ...
 */
export const pwaMeSummary = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
export const pwaMeGoals = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
...
function calculateQuizProfile(answers: Array<{ questionId: string; answer: unknown }>) {
...
function calculateCoachingRecommendations(
  packages: Array<{ id: string; [key: string]: unknown }>,
  quizProfile: Record<string, unknown> | undefined,
