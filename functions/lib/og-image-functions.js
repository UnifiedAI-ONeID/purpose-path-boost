 ...
 */
exports.ogRenderAll = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
    ...
    const { title, subtitle, slug, theme = 'light', lang = 'en', tag = 'mindset' } = data || {};
    if (!title || !slug) {
 ...
 */
exports.ogRenderSingle = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
...
//# sourceMappingURL=og-image-functions.js.map